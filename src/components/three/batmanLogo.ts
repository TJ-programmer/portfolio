...
/* Classic Batman logo silhouette (black outer contour of the iconic
   bat emblem). Single closed <path>, width normalized to ~5.4 world
   units so the extruded emblem keeps the proportions used by the rest
   of the scene. */
const BATMAN_LOGO_PATH =
...
export function batLogoShapes(): THREE.Shape[] {
  if (cached) return cached;
  const parsed = new SVGLoader().parse(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 600"><path d="${BATMAN_LOGO_PATH}"/></svg>`,
  );
  const shapes = SVGLoader.createShapes(parsed.paths[0]);
  const raw = shapes[0].getPoints(8);
  let minX = Infinity;
  let maxX = -Infinity;
  for (const p of raw) {
  ...
  const k = 5.4 / (maxX - minX);
  for (const shape of shapes) {
    for (const c of shape.curves) {
      if (c instanceof THREE.LineCurve) {
        c.v1.multiplyScalar(k);
        c.v2.multiplyScalar(k);
      } else if (c instanceof THREE.CubicBezierCurve) {
        c.v0.multiplyScalar(k);
        c.v1.multiplyScalar(k);
        c.v2.multiplyScalar(k);
        c.v3.multiplyScalar(k);
      } else if (c instanceof THREE.QuadraticBezierCurve) {
        c.v0.multiplyScalar(k);
        c.v1.multiplyScalar(k);
        c.v2.multiplyScalar(k);
      } else if (c instanceof THREE.EllipseCurve) {
        c.aX *= k;
        c.aY *= k;
        c.xRadius *= k;
        c.yRadius *= k;
